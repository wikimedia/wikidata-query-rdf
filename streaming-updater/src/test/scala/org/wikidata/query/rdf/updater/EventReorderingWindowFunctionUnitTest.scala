...
class EventReorderingWindowFunctionUnitTest extends FlatSpec with Matchers with TestEventGenerator {
  "EventReorderingWindowFunction" should "reorder events" in {
    ...
    operator.open()
    operator.processElement(newInputEventRecord("Q1", 2, 0))
    operator.processElement(newInputEventRecord("Q1", 1, 1))
    operator.processElement(newInputEventRecord("Q2", 1, 0))
    operator.processElement(newInputEventRecord("Q2", 2, 1))
    operator.processWatermark(new Watermark(1000))
    operator.processElement(newInputEventRecord("Q2", 3, 2)) // Late event
    expectedOutput += newInputEventRecord("Q1", 1, 1)
    expectedOutput += newInputEventRecord("Q1", 2, 0)
    expectedOutput += newInputEventRecord("Q2", 1, 0)
    expectedOutput += newInputEventRecord("Q2", 2, 1)
